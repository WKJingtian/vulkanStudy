...
vulkanSwapchain::vulkanSwapchain(vulkanDevice &dev, VkExtent2D extent)
    : device{dev}, windowExtent{extent}
{ init(); }

vulkanSwapchain::vulkanSwapchain(
    vulkanDevice &dev, VkExtent2D extent, std::shared_ptr<vulkanSwapchain> previous)
    : device{dev}, windowExtent{extent}, oldSwapChain{previous}
{ init(); oldSwapChain = nullptr; }
...
{
    vkWaitForFences(
        device.device, 1, &inFlightFences[currentFrame],
...
VkResult vulkanSwapchain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t *imageIndex)
{
    ...
    if (vkQueueSubmit(device.graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) !=
        VK_SUCCESS) throw std::runtime_error("failed to submit draw command buffer!");
...
void vulkanSwapchain::createImageViews()
{
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++)
    {
        ...
        if (vkCreateImageView(device.device, &viewInfo, nullptr, &swapChainImageViews[i]) !=
            VK_SUCCESS) throw std::runtime_error("failed to create texture image view!");
    }
}
